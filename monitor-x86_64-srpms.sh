...
do
    ...
    # 根据实际情况修改命令路径或参数
    ssh "root@192.168.235.116" "/root/rpmbuild/SPECS/rbb /mnt/magicx86/$NEW_FILE"
    RENO=$?
    if [ $RENO == 0 ]; then
	    #cp $NEW_FILE /mnt/loongarch64/root/rpmbuild/SRPMS/$(basename $NEW_FILE) -fv
	    echo "$NEW_FILE" >> rebuildfromx64
    else
	    echo "编译出错，请检查并手工处理"
    fi
done
